...
DOCUMENTATION = '''
---
module: fortios_firewall.shaper_traffic-shaper
short_description: Configure shared traffic shaper.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure firewall.shaper feature and traffic-shaper category.
...
- hosts: localhost
  ...
  - name: Configure shared traffic shaper.
    fortios_firewall.shaper_traffic-shaper:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_firewall.shaper_traffic-shaper_data(json):
...
def firewall.shaper_traffic-shaper(data, fos):
    vdom = data['vdom']
    firewall.shaper_traffic-shaper_data = data['firewall.shaper_traffic-shaper']
    filtered_data = filter_firewall.shaper_traffic - \
        shaper_data(firewall.shaper_traffic-shaper_data)

    if firewall.shaper_traffic-shaper_data['state'] == "present":
        return fos.set('firewall.shaper',
                       'traffic-shaper',
                       data=filtered_data,
                       vdom=vdom)

    elif firewall.shaper_traffic-shaper_data['state'] == "absent":
...
def fortios_firewall.shaper(data, fos):
    ...
    methodlist = ['firewall.shaper_traffic-shaper']
