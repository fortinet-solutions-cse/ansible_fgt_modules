  ...
  - name: Local keys and certificates.
    fortios_vpn_certificate_local:
      ...
      vpn_certificate_local:
        ...
        cmp-server-cert: "<your_own_value> (source vpn.certificate.ca.name)"
...
def vpn_certificate_local(data, fos):
    ...
    flattened_data = flatten_multilists_attributes(vpn_certificate_local_data)
    filtered_data = filter_vpn_certificate_local_data(flattened_data)
    if vpn_certificate_local_data['state'] == "present":
        return fos.set('vpn.certificate',
                       'local',
                       data=filtered_data,
                       vdom=vdom)

    elif vpn_certificate_local_data['state'] == "absent":
...
def fortios_vpn_certificate(data, fos):
    ...
    methodlist = ['vpn_certificate_local']
