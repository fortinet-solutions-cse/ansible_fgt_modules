...
DOCUMENTATION = '''
---
module: fortios_switch-controller.qos_dot1p-map
short_description: Configure FortiSwitch QoS 802.1p.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure switch-controller.qos feature and dot1p-map category.
...
- hosts: localhost
  ...
  - name: Configure FortiSwitch QoS 802.1p.
    fortios_switch-controller.qos_dot1p-map:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_switch-controller.qos_dot1p-map_data(json):
...
def switch-controller.qos_dot1p-map(data, fos):
    vdom = data['vdom']
    switch-controller.qos_dot1p-map_data = data['switch-controller.qos_dot1p-map']
    filtered_data = filter_switch-controller.qos_dot1p - \
        map_data(switch-controller.qos_dot1p-map_data)

    if switch-controller.qos_dot1p-map_data['state'] == "present":
        return fos.set('switch-controller.qos',
                       'dot1p-map',
                       data=filtered_data,
                       vdom=vdom)

    elif switch-controller.qos_dot1p-map_data['state'] == "absent":
...
def fortios_switch-controller.qos(data, fos):
    ...
    methodlist = ['switch-controller.qos_dot1p-map']
