...
DOCUMENTATION = '''
---
module: fortios_system.replacemsg_nntp
short_description: Replacement messages in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure system.replacemsg feature and nntp category.
...
def filter_system.replacemsg_nntp_data(json):
...
def system.replacemsg_nntp(data, fos):
    vdom = data['vdom']
    system.replacemsg_nntp_data = data['system.replacemsg_nntp']
    filtered_data = filter_system.replacemsg_nntp_data(system.replacemsg_nntp_data)
    if system.replacemsg_nntp_data['state'] == "present":
        return fos.set('system.replacemsg',
                       'nntp',
                       data=filtered_data,
                       vdom=vdom)

    elif system.replacemsg_nntp_data['state'] == "absent":
...
def fortios_system.replacemsg(data, fos):
    ...
    methodlist = ['system.replacemsg_nntp']
