...
DOCUMENTATION = '''
---
module: fortios_switch-controller.security-policy_802-1X
short_description: Configure 802.1x MAC Authentication Bypass (MAB) policies.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure switch-controller.security-policy feature and 802-1X category.
...
- hosts: localhost
  ...
  - name: Configure 802.1x MAC Authentication Bypass (MAB) policies.
    fortios_switch-controller.security-policy_802-1X:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_switch-controller.security-policy_802-1X_data(json):
...
def switch-controller.security-policy_802-1X(data, fos):
    vdom = data['vdom']
    switch-controller.security-policy_802-1X_data = data['switch-controller.security-policy_802-1X']
    filtered_data = filter_switch-controller.security-policy_802-1X_data(switch-controller.security-policy_802-1X_data)

    if switch-controller.security-policy_802-1X_data['state'] == "present":
        return fos.set('switch-controller.security-policy',
                       '802-1X',
                       data=filtered_data,
                       vdom=vdom)

    elif switch-controller.security-policy_802-1X_data['state'] == "absent":
...
def fortios_switch-controller.security-policy(data, fos):
    ...
    methodlist = ['switch-controller.security-policy_802-1X']
