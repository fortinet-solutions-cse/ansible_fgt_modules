...
DOCUMENTATION = '''
---
module: fortios_system_virtual-wire-pair
short_description: Configure virtual wire pairs.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure system feature and virtual-wire-pair category.
...
- hosts: localhost
  ...
  - name: Configure virtual wire pairs.
    fortios_system_virtual-wire-pair:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_system_virtual-wire-pair_data(json):
...
def system_virtual-wire-pair(data, fos):
    vdom = data['vdom']
    system_virtual-wire-pair_data = data['system_virtual-wire-pair']
    filtered_data = filter_system_virtual-wire - \
        pair_data(system_virtual-wire-pair_data)

    if system_virtual-wire-pair_data['state'] == "present":
        return fos.set('system',
                       'virtual-wire-pair',
                       data=filtered_data,
                       vdom=vdom)

    elif system_virtual-wire-pair_data['state'] == "absent":
...
def fortios_system(data, fos):
    ...
    methodlist = ['system_virtual-wire-pair']
