...
DOCUMENTATION = '''
---
module: fortios_vpn.ipsec_phase2-interface
short_description: Configure VPN autokey tunnel.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure vpn.ipsec feature and phase2-interface category.
...
- hosts: localhost
  ...
  - name: Configure VPN autokey tunnel.
    fortios_vpn.ipsec_phase2-interface:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_vpn.ipsec_phase2-interface_data(json):
...
def vpn.ipsec_phase2-interface(data, fos):
    vdom = data['vdom']
    vpn.ipsec_phase2-interface_data = data['vpn.ipsec_phase2-interface']
    filtered_data = filter_vpn.ipsec_phase2 - \
        interface_data(vpn.ipsec_phase2-interface_data)

    if vpn.ipsec_phase2-interface_data['state'] == "present":
        return fos.set('vpn.ipsec',
                       'phase2-interface',
                       data=filtered_data,
                       vdom=vdom)

    elif vpn.ipsec_phase2-interface_data['state'] == "absent":
...
def fortios_vpn.ipsec(data, fos):
    ...
    methodlist = ['vpn.ipsec_phase2-interface']
