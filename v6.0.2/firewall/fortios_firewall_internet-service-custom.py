...
DOCUMENTATION = '''
---
module: fortios_firewall_internet-service-custom
short_description: Configure custom Internet Services.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure firewall feature and internet-service-custom category.
...
- hosts: localhost
  ...
  - name: Configure custom Internet Services.
    fortios_firewall_internet-service-custom:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_firewall_internet-service-custom_data(json):
...
def firewall_internet-service-custom(data, fos):
    vdom = data['vdom']
    firewall_internet-service-custom_data = data['firewall_internet-service-custom']
    filtered_data = filter_firewall_internet-service - \
        custom_data(firewall_internet-service-custom_data)

    if firewall_internet-service-custom_data['state'] == "present":
        return fos.set('firewall',
                       'internet-service-custom',
                       data=filtered_data,
                       vdom=vdom)

    elif firewall_internet-service-custom_data['state'] == "absent":
...
def fortios_firewall(data, fos):
    ...
    methodlist = ['firewall_internet-service-custom']
                ...
                "disable-entry": {"required": False, "type": "list",
                                  "options": {
                                      ...
                                      "ip-range": {"required": False, "type": "list",
                                                   "options": {
                                                       "end-ip": {"required": False, "type": "ipv4-address-any"},
                                                       "id": {"required": True, "type": "int"},
                                                       "start-ip": {"required": False, "type": "ipv4-address-any"}
                                                   }},
