...
DOCUMENTATION = '''
---
module: fortios_firewall_interface-policy
short_description: Configure IPv4 interface policies.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure firewall feature and interface-policy category.
...
- hosts: localhost
  ...
  - name: Configure IPv4 interface policies.
    fortios_firewall_interface-policy:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_firewall_interface-policy_data(json):
...
def firewall_interface-policy(data, fos):
    vdom = data['vdom']
    firewall_interface-policy_data = data['firewall_interface-policy']
    filtered_data = filter_firewall_interface - \
        policy_data(firewall_interface-policy_data)

    if firewall_interface-policy_data['state'] == "present":
        return fos.set('firewall',
                       'interface-policy',
                       data=filtered_data,
                       vdom=vdom)

    elif firewall_interface-policy_data['state'] == "absent":
...
def fortios_firewall(data, fos):
    ...
    methodlist = ['firewall_interface-policy']
