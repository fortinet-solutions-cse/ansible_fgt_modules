...
DOCUMENTATION = '''
---
module: fortios_firewall.shaper_per-ip-shaper
short_description: Configure per-IP traffic shaper.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure firewall.shaper feature and per-ip-shaper category.
...
- hosts: localhost
  ...
  - name: Configure per-IP traffic shaper.
    fortios_firewall.shaper_per-ip-shaper:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_firewall.shaper_per-ip-shaper_data(json):
...
def firewall.shaper_per-ip-shaper(data, fos):
    vdom = data['vdom']
    firewall.shaper_per-ip-shaper_data = data['firewall.shaper_per-ip-shaper']
    filtered_data = filter_firewall.shaper_per-ip - \
        shaper_data(firewall.shaper_per-ip-shaper_data)

    if firewall.shaper_per-ip-shaper_data['state'] == "present":
        return fos.set('firewall.shaper',
                       'per-ip-shaper',
                       data=filtered_data,
                       vdom=vdom)

    elif firewall.shaper_per-ip-shaper_data['state'] == "absent":
...
def fortios_firewall.shaper(data, fos):
    ...
    methodlist = ['firewall.shaper_per-ip-shaper']
