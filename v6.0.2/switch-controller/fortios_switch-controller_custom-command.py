...
DOCUMENTATION = '''
---
module: fortios_switch-controller_custom-command
...
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure switch-controller feature and custom-command category.
...
- hosts: localhost
  ...
  - name: Configure the FortiGate switch controller to send custom commands to managed FortiSwitch devices.
    fortios_switch-controller_custom-command:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_switch-controller_custom-command_data(json):
    option_list = ['command', 'command-name', 'description']
    dictionary = {}
...
def switch-controller_custom-command(data, fos):
    vdom = data['vdom']
    switch-controller_custom-command_data = data['switch-controller_custom-command']
    filtered_data = filter_switch-controller_custom - \
        command_data(switch-controller_custom-command_data)

    if switch-controller_custom-command_data['state'] == "present":
        return fos.set('switch-controller',
                       'custom-command',
                       data=filtered_data,
                       vdom=vdom)

    elif switch-controller_custom-command_data['state'] == "absent":
...
def fortios_switch-controller(data, fos):
    ...
    methodlist = ['switch-controller_custom-command']
