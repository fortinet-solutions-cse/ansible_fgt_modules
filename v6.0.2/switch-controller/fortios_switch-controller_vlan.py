...
DOCUMENTATION = '''
---
module: fortios_switch-controller_vlan
short_description: Configure VLANs for switch controller.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure switch-controller feature and vlan category.
...
- hosts: localhost
  ...
  - name: Configure VLANs for switch controller.
    fortios_switch-controller_vlan:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_switch-controller_vlan_data(json):
...
def switch-controller_vlan(data, fos):
    vdom = data['vdom']
    switch-controller_vlan_data = data['switch-controller_vlan']
    filtered_data = filter_switch - \
        controller_vlan_data(switch-controller_vlan_data)

    if switch-controller_vlan_data['state'] == "present":
        return fos.set('switch-controller',
                       'vlan',
                       data=filtered_data,
                       vdom=vdom)

    elif switch-controller_vlan_data['state'] == "absent":
...
def fortios_switch-controller(data, fos):
    ...
    methodlist = ['switch-controller_vlan']
