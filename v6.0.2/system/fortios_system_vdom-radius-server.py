...
DOCUMENTATION = '''
---
module: fortios_system_vdom-radius-server
short_description: Configure a RADIUS server to use as a RADIUS Single Sign On (RSSO) server for this VDOM.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure system feature and vdom-radius-server category.
...
- hosts: localhost
  ...
  - name: Configure a RADIUS server to use as a RADIUS Single Sign On (RSSO) server for this VDOM.
    fortios_system_vdom-radius-server:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_system_vdom-radius-server_data(json):
    option_list = ['name', 'radius-server-vdom', 'status']
    dictionary = {}
...
def system_vdom-radius-server(data, fos):
    vdom = data['vdom']
    system_vdom-radius-server_data = data['system_vdom-radius-server']
    filtered_data = filter_system_vdom-radius - \
        server_data(system_vdom-radius-server_data)

    if system_vdom-radius-server_data['state'] == "present":
        return fos.set('system',
                       'vdom-radius-server',
                       data=filtered_data,
                       vdom=vdom)

    elif system_vdom-radius-server_data['state'] == "absent":
...
def fortios_system(data, fos):
    ...
    methodlist = ['system_vdom-radius-server']
    ...
    fields = {
        ...
        "system_vdom-radius-server": {
            ...
            "options": {
                ...
                "radius-server-vdom": {"required": False, "type": "str"},
