...
DOCUMENTATION = '''
---
module: fortios_endpoint-control_settings
short_description: Configure endpoint control settings.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure endpoint-control feature and settings category.
...
- hosts: localhost
  ...
  - name: Configure endpoint control settings.
    fortios_endpoint-control_settings:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_endpoint-control_settings_data(json):
...
def endpoint-control_settings(data, fos):
    vdom = data['vdom']
    endpoint-control_settings_data = data['endpoint-control_settings']
    filtered_data = filter_endpoint - \
        control_settings_data(endpoint-control_settings_data)

    if endpoint-control_settings_data['state'] == "present":
        return fos.set('endpoint-control',
                       'settings',
                       data=filtered_data,
                       vdom=vdom)

    elif endpoint-control_settings_data['state'] == "absent":
...
def fortios_endpoint-control(data, fos):
    ...
    methodlist = ['endpoint-control_settings']
    ...
    fields = {
        ...
        "endpoint-control_settings": {
            ...
            "options": {
                ...
                "forticlient-reg-key": {"required": False, "type": "password"},
