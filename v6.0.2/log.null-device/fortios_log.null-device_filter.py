...
DOCUMENTATION = '''
---
module: fortios_log.null-device_filter
short_description: Filters for null device logging.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure log.null-device feature and filter category.
...
- hosts: localhost
  ...
  - name: Filters for null device logging.
    fortios_log.null-device_filter:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_log.null-device_filter_data(json):
...
def log.null-device_filter(data, fos):
    vdom = data['vdom']
    log.null-device_filter_data = data['log.null-device_filter']
    filtered_data = filter_log.null - \
        device_filter_data(log.null-device_filter_data)

    if log.null-device_filter_data['state'] == "present":
        return fos.set('log.null-device',
                       'filter',
                       data=filtered_data,
                       vdom=vdom)

    elif log.null-device_filter_data['state'] == "absent":
...
def fortios_log.null-device(data, fos):
    ...
    methodlist = ['log.null-device_filter']
