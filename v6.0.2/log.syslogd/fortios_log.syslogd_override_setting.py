...
- hosts: localhost
  ...
  - name: Override settings for remote syslog server.
    fortios_log.syslogd_override_setting:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def filter_log.syslogd_override_setting_data(json):
...
def fortios_log.syslogd(data, fos):
    ...
    methodlist = ['log.syslogd_override_setting']
