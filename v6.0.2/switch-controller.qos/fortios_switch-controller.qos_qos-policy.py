...
DOCUMENTATION = '''
---
module: fortios_switch-controller.qos_qos-policy
short_description: Configure FortiSwitch QoS policy.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure switch-controller.qos feature and qos-policy category.
...
- hosts: localhost
  ...
  - name: Configure FortiSwitch QoS policy.
    fortios_switch-controller.qos_qos-policy:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_switch-controller.qos_qos-policy_data(json):
...
def switch-controller.qos_qos-policy(data, fos):
    vdom = data['vdom']
    switch-controller.qos_qos-policy_data = data['switch-controller.qos_qos-policy']
    filtered_data = filter_switch-controller.qos_qos - \
        policy_data(switch-controller.qos_qos-policy_data)

    if switch-controller.qos_qos-policy_data['state'] == "present":
        return fos.set('switch-controller.qos',
                       'qos-policy',
                       data=filtered_data,
                       vdom=vdom)

    elif switch-controller.qos_qos-policy_data['state'] == "absent":
...
def fortios_switch-controller.qos(data, fos):
    ...
    methodlist = ['switch-controller.qos_qos-policy']
