...
- hosts: localhost
  ...
  - name: Filters for FortiAnalyzer.
    fortios_log.fortianalyzer3_filter:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_log.fortianalyzer3_filter_data(json):
...
def log.fortianalyzer3_filter(data, fos):
    ...
    if log.fortianalyzer3_filter_data['state'] == "present":
        return fos.set('log.fortianalyzer3',
                       'filter',
                       data=filtered_data,
                       vdom=vdom)

    elif log.fortianalyzer3_filter_data['state'] == "absent":
...
def fortios_log.fortianalyzer3(data, fos):
    ...
    methodlist = ['log.fortianalyzer3_filter']
