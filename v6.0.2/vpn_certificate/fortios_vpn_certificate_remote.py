...
def filter_vpn_certificate_remote_data(json):
    option_list = ['name', 'range', 'remote',
                   'source']
    dictionary = {}
...
def vpn_certificate_remote(data, fos):
    vdom = data['vdom']
    vpn_certificate_remote_data = data['vpn_certificate_remote']
    flattened_data = flatten_multilists_attributes(vpn_certificate_remote_data)
    filtered_data = filter_vpn_certificate_remote_data(flattened_data)
    if vpn_certificate_remote_data['state'] == "present":
        return fos.set('vpn.certificate',
                       'remote',
                       data=filtered_data,
                       vdom=vdom)

    elif vpn_certificate_remote_data['state'] == "absent":
