...
DOCUMENTATION = '''
---
module: fortios_firewall_multicast-address
short_description: Configure multicast addresses.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure firewall feature and multicast-address category.
...
- hosts: localhost
  ...
  - name: Configure multicast addresses.
    fortios_firewall_multicast-address:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_firewall_multicast-address_data(json):
...
def firewall_multicast-address(data, fos):
    vdom = data['vdom']
    firewall_multicast-address_data = data['firewall_multicast-address']
    filtered_data = filter_firewall_multicast - \
        address_data(firewall_multicast-address_data)

    if firewall_multicast-address_data['state'] == "present":
        return fos.set('firewall',
                       'multicast-address',
                       data=filtered_data,
                       vdom=vdom)

    elif firewall_multicast-address_data['state'] == "absent":
...
def fortios_firewall(data, fos):
    ...
    methodlist = ['firewall_multicast-address']
    ...
    fields = {
        ...
        "firewall_multicast-address": {
            ...
            "options": {
                ...
                "subnet": {"required": False, "type": "ipv4-classnet-any"},
