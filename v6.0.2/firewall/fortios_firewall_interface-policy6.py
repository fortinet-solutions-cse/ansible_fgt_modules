...
DOCUMENTATION = '''
---
module: fortios_firewall_interface-policy6
short_description: Configure IPv6 interface policies.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure firewall feature and interface-policy6 category.
...
- hosts: localhost
  ...
  - name: Configure IPv6 interface policies.
    fortios_firewall_interface-policy6:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_firewall_interface-policy6_data(json):
...
def firewall_interface-policy6(data, fos):
    vdom = data['vdom']
    firewall_interface-policy6_data = data['firewall_interface-policy6']
    filtered_data = filter_firewall_interface - \
        policy6_data(firewall_interface-policy6_data)

    if firewall_interface-policy6_data['state'] == "present":
        return fos.set('firewall',
                       'interface-policy6',
                       data=filtered_data,
                       vdom=vdom)

    elif firewall_interface-policy6_data['state'] == "absent":
...
def fortios_firewall(data, fos):
    ...
    methodlist = ['firewall_interface-policy6']
