...
DOCUMENTATION = '''
---
module: fortios_system.replacemsg_utm
short_description: Replacement messages in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure system.replacemsg feature and utm category.
...
def filter_system.replacemsg_utm_data(json):
...
def system.replacemsg_utm(data, fos):
    vdom = data['vdom']
    system.replacemsg_utm_data = data['system.replacemsg_utm']
    filtered_data = filter_system.replacemsg_utm_data(system.replacemsg_utm_data)
    if system.replacemsg_utm_data['state'] == "present":
        return fos.set('system.replacemsg',
                       'utm',
                       data=filtered_data,
                       vdom=vdom)

    elif system.replacemsg_utm_data['state'] == "absent":
...
def fortios_system.replacemsg(data, fos):
    ...
    methodlist = ['system.replacemsg_utm']
