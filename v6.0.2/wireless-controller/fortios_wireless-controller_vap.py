...
DOCUMENTATION = '''
---
module: fortios_wireless-controller_vap
short_description: Configure Virtual Access Points (VAPs).
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure wireless-controller feature and vap category.
...
- hosts: localhost
  ...
  - name: Configure Virtual Access Points (VAPs).
    fortios_wireless-controller_vap:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_wireless-controller_vap_data(json):
...
def wireless-controller_vap(data, fos):
    vdom = data['vdom']
    wireless-controller_vap_data = data['wireless-controller_vap']
    filtered_data = filter_wireless - \
        controller_vap_data(wireless-controller_vap_data)

    if wireless-controller_vap_data['state'] == "present":
        return fos.set('wireless-controller',
                       'vap',
                       data=filtered_data,
                       vdom=vdom)

    elif wireless-controller_vap_data['state'] == "absent":
...
def fortios_wireless-controller(data, fos):
    ...
    methodlist = ['wireless-controller_vap']
    ...
    fields = {
        ...
        "wireless-controller_vap": {
            ...
            "options": {
                ...
                "captive-portal-macauth-radius-secret": {"required": False, "type": "password"},
                "captive-portal-macauth-radius-server": {"required": False, "type": "str"},
                "captive-portal-radius-secret": {"required": False, "type": "password"},
                ...
                "mpsk-key": {"required": False, "type": "list",
                             "options": {
                                 ...
                                 "key-name": {"required": False, "type": "str"},
                                 "passphrase": {"required": False, "type": "password"}
                             }},
