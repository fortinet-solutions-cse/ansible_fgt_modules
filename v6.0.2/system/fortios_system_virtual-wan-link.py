...
DOCUMENTATION = '''
---
module: fortios_system_virtual-wan-link
short_description: Configure redundant internet connections using SD-WAN (formerly virtual WAN link).
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure system feature and virtual-wan-link category.
...
- hosts: localhost
  ...
  - name: Configure redundant internet connections using SD-WAN (formerly virtual WAN link).
    fortios_system_virtual-wan-link:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_system_virtual-wan-link_data(json):
...
def system_virtual-wan-link(data, fos):
    vdom = data['vdom']
    system_virtual-wan-link_data = data['system_virtual-wan-link']
    filtered_data = filter_system_virtual-wan - \
        link_data(system_virtual-wan-link_data)

    if system_virtual-wan-link_data['state'] == "present":
        return fos.set('system',
                       'virtual-wan-link',
                       data=filtered_data,
                       vdom=vdom)

    elif system_virtual-wan-link_data['state'] == "absent":
...
def fortios_system(data, fos):
    ...
    methodlist = ['system_virtual-wan-link']
            ...
            "options": {
                ...
                "members": {"required": False, "type": "list",
                            "options": {
                                ...
                                "seq-num": {"required": False, "type": "int"},
                                "source": {"required": False, "type": "ipv4-address"},
                                "source6": {"required": False, "type": "ipv6-address"},
