...
DOCUMENTATION = '''
---
module: fortios_switch-controller_lldp-profile
short_description: Configure FortiSwitch LLDP profiles.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure switch-controller feature and lldp-profile category.
...
- hosts: localhost
  ...
  - name: Configure FortiSwitch LLDP profiles.
    fortios_switch-controller_lldp-profile:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_switch-controller_lldp-profile_data(json):
...
def switch-controller_lldp-profile(data, fos):
    vdom = data['vdom']
    switch-controller_lldp-profile_data = data['switch-controller_lldp-profile']
    filtered_data = filter_switch-controller_lldp - \
        profile_data(switch-controller_lldp-profile_data)

    if switch-controller_lldp-profile_data['state'] == "present":
        return fos.set('switch-controller',
                       'lldp-profile',
                       data=filtered_data,
                       vdom=vdom)

    elif switch-controller_lldp-profile_data['state'] == "absent":
...
def fortios_switch-controller(data, fos):
    ...
    methodlist = ['switch-controller_lldp-profile']
