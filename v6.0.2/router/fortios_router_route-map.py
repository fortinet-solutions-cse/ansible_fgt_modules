...
DOCUMENTATION = '''
---
module: fortios_router_route-map
short_description: Configure route maps.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure router feature and route-map category.
...
def filter_router_route-map_data(json):
...
def router_route-map(data, fos):
    vdom = data['vdom']
    router_route-map_data = data['router_route-map']
    filtered_data = filter_router_route-map_data(router_route-map_data)

    if router_route-map_data['state'] == "present":
        return fos.set('router',
                       'route-map',
                       data=filtered_data,
                       vdom=vdom)

    elif router_route-map_data['state'] == "absent":
            ...
            "options": {
                ...
                "rule": {"required": False, "type": "list",
                         "options": {
                             ...
                             "set-aggregator-ip": {"required": False, "type": "ipv4-address-any"},
                             "set-aspath": {"required": False, "type": "list",
                                            "options": {
                                                "as": {"required": False, "type": "str"}
                                            }},
                             ...
                             "set-community": {"required": False, "type": "list",
                                               "options": {
                                                   "community": {"required": False, "type": "str"}
                                               }},
                             ...
                             "set-ip-nexthop": {"required": False, "type": "ipv4-address"},
                             "set-ip6-nexthop": {"required": False, "type": "ipv6-address"},
                             "set-ip6-nexthop-local": {"required": False, "type": "ipv6-address"},
                             ...
                             "set-origin": {"required": False, "type": "str",
                                            "choices": ["none", "egp", "igp",
                                                        "incomplete"]},
                             "set-originator-id": {"required": False, "type": "ipv4-address-any"},
