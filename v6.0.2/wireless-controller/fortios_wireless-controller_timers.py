...
DOCUMENTATION = '''
---
module: fortios_wireless-controller_timers
short_description: Configure CAPWAP timers.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure wireless-controller feature and timers category.
...
- hosts: localhost
  ...
  - name: Configure CAPWAP timers.
    fortios_wireless-controller_timers:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_wireless-controller_timers_data(json):
...
def wireless-controller_timers(data, fos):
    vdom = data['vdom']
    wireless-controller_timers_data = data['wireless-controller_timers']
    filtered_data = filter_wireless - \
        controller_timers_data(wireless-controller_timers_data)

    if wireless-controller_timers_data['state'] == "present":
        return fos.set('wireless-controller',
                       'timers',
                       data=filtered_data,
                       vdom=vdom)

    elif wireless-controller_timers_data['state'] == "absent":
...
def fortios_wireless-controller(data, fos):
    ...
    methodlist = ['wireless-controller_timers']
