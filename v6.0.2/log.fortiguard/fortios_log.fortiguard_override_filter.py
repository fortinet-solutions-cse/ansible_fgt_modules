...
options:
    ...
    https:
        ...
        type: bool
        default: false
    log.fortiguard_override_filter:
        description:
...
- hosts: localhost
  ...
  - name: Override filters for FortiCloud.
    fortios_log.fortiguard_override_filter:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def filter_log.fortiguard_override_filter_data(json):
...
def fortios_log.fortiguard(data, fos):
    ...
    methodlist = ['log.fortiguard_override_filter']
