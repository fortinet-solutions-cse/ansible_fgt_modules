...
DOCUMENTATION = '''
---
module: fortios_wanopt_cache-service
short_description: Designate cache-service for wan-optimization and webcache.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure wanopt feature and cache-service category.
...
def filter_wanopt_cache-service_data(json):
...
def wanopt_cache-service(data, fos):
    vdom = data['vdom']
    wanopt_cache-service_data = data['wanopt_cache-service']
    filtered_data = filter_wanopt_cache-service_data(wanopt_cache-service_data)

    if wanopt_cache-service_data['state'] == "present":
        return fos.set('wanopt',
                       'cache-service',
                       data=filtered_data,
                       vdom=vdom)

    elif wanopt_cache-service_data['state'] == "absent":
...
def fortios_wanopt(data, fos):
    ...
    methodlist = ['wanopt_cache-service']
