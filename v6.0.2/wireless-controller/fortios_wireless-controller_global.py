...
DOCUMENTATION = '''
---
module: fortios_wireless-controller_global
short_description: Configure wireless controller global settings.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure wireless-controller feature and global category.
...
- hosts: localhost
  ...
  - name: Configure wireless controller global settings.
    fortios_wireless-controller_global:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_wireless-controller_global_data(json):
...
def wireless-controller_global(data, fos):
    vdom = data['vdom']
    wireless-controller_global_data = data['wireless-controller_global']
    filtered_data = filter_wireless - \
        controller_global_data(wireless-controller_global_data)

    if wireless-controller_global_data['state'] == "present":
        return fos.set('wireless-controller',
                       'global',
                       data=filtered_data,
                       vdom=vdom)

    elif wireless-controller_global_data['state'] == "absent":
...
def fortios_wireless-controller(data, fos):
    ...
    methodlist = ['wireless-controller_global']
    ...
    fields = {
        ...
        "wireless-controller_global": {
            ...
            "options": {
                ...
                "ap-log-server-ip": {"required": False, "type": "ipv4-address"},
                ...
                "image-download": {"required": False, "type": "str",
                                   "choices": ["enable", "disable"]},
                "ipsec-base-ip": {"required": False, "type": "ipv4-address"},
