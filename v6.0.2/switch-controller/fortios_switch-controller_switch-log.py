...
DOCUMENTATION = '''
---
module: fortios_switch-controller_switch-log
short_description: Configure FortiSwitch logging (logs are transferred to and inserted into FortiGate event log).
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure switch-controller feature and switch-log category.
...
- hosts: localhost
  ...
  - name: Configure FortiSwitch logging (logs are transferred to and inserted into FortiGate event log).
    fortios_switch-controller_switch-log:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_switch-controller_switch-log_data(json):
    option_list = ['severity', 'status']
    dictionary = {}
...
def switch-controller_switch-log(data, fos):
    vdom = data['vdom']
    switch-controller_switch-log_data = data['switch-controller_switch-log']
    filtered_data = filter_switch-controller_switch - \
        log_data(switch-controller_switch-log_data)

    if switch-controller_switch-log_data['state'] == "present":
        return fos.set('switch-controller',
                       'switch-log',
                       data=filtered_data,
                       vdom=vdom)

    elif switch-controller_switch-log_data['state'] == "absent":
...
def fortios_switch-controller(data, fos):
    ...
    methodlist = ['switch-controller_switch-log']
