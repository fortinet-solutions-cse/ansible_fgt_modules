...
DOCUMENTATION = '''
---
module: fortios_firewall.ssh_local-ca
short_description: SSH proxy local CA.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure firewall.ssh feature and local-ca category.
...
def filter_firewall.ssh_local-ca_data(json):
...
def firewall.ssh_local-ca(data, fos):
    vdom = data['vdom']
    firewall.ssh_local-ca_data = data['firewall.ssh_local-ca']
    filtered_data = filter_firewall.ssh_local - \
        ca_data(firewall.ssh_local-ca_data)

    if firewall.ssh_local-ca_data['state'] == "present":
        return fos.set('firewall.ssh',
                       'local-ca',
                       data=filtered_data,
                       vdom=vdom)

    elif firewall.ssh_local-ca_data['state'] == "absent":
...
def fortios_firewall.ssh(data, fos):
    ...
    methodlist = ['firewall.ssh_local-ca']
