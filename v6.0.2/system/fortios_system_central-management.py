...
DOCUMENTATION = '''
---
module: fortios_system_central-management
short_description: Configure central management.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure system feature and central-management category.
...
- hosts: localhost
  ...
  - name: Configure central management.
    fortios_system_central-management:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_system_central-management_data(json):
...
def system_central-management(data, fos):
    vdom = data['vdom']
    system_central-management_data = data['system_central-management']
    filtered_data = filter_system_central - \
        management_data(system_central-management_data)

    if system_central-management_data['state'] == "present":
        return fos.set('system',
                       'central-management',
                       data=filtered_data,
                       vdom=vdom)

    elif system_central-management_data['state'] == "absent":
...
def fortios_system(data, fos):
    ...
    methodlist = ['system_central-management']
    ...
    fields = {
        ...
        "system_central-management": {
            ...
            "options": {
                ...
                "fmg-source-ip": {"required": False, "type": "ipv4-address"},
                "fmg-source-ip6": {"required": False, "type": "ipv6-address"},
                ...
                "server-list": {"required": False, "type": "list",
                                "options": {
                                    ...
                                    "server-address": {"required": False, "type": "ipv4-address"},
                                    "server-address6": {"required": False, "type": "ipv6-address"},
