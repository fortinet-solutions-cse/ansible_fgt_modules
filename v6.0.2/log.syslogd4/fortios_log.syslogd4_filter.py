...
DOCUMENTATION = '''
---
module: fortios_log.syslogd4_filter
short_description: Filters for remote system server.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure log.syslogd4 feature and filter category.
...
def filter_log.syslogd4_filter_data(json):
...
def log.syslogd4_filter(data, fos):
    vdom = data['vdom']
    log.syslogd4_filter_data = data['log.syslogd4_filter']
    filtered_data = filter_log.syslogd4_filter_data(log.syslogd4_filter_data)

    if log.syslogd4_filter_data['state'] == "present":
        return fos.set('log.syslogd4',
                       'filter',
                       data=filtered_data,
                       vdom=vdom)

    elif log.syslogd4_filter_data['state'] == "absent":
        return fos.delete('log.syslogd4',
                          'filter',
                          mkey=filtered_data['id'],
                          vdom=vdom)


def fortios_log.syslogd4(data, fos):
    host = data['host']
    username = data['username']
    password = data['password']
    fos.https('off')
    ...
    methodlist = ['log.syslogd4_filter']
...
def main():
    ...
    is_error, has_changed, result = fortios_log.syslogd4(module.params, fos)
