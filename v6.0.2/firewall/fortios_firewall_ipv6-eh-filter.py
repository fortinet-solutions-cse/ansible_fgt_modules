...
DOCUMENTATION = '''
---
module: fortios_firewall_ipv6-eh-filter
short_description: Configure IPv6 extension header filter.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure firewall feature and ipv6-eh-filter category.
...
- hosts: localhost
  ...
  - name: Configure IPv6 extension header filter.
    fortios_firewall_ipv6-eh-filter:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_firewall_ipv6-eh-filter_data(json):
...
def firewall_ipv6-eh-filter(data, fos):
    vdom = data['vdom']
    firewall_ipv6-eh-filter_data = data['firewall_ipv6-eh-filter']
    filtered_data = filter_firewall_ipv6-eh - \
        filter_data(firewall_ipv6-eh-filter_data)

    if firewall_ipv6-eh-filter_data['state'] == "present":
        return fos.set('firewall',
                       'ipv6-eh-filter',
                       data=filtered_data,
                       vdom=vdom)

    elif firewall_ipv6-eh-filter_data['state'] == "absent":
...
def fortios_firewall(data, fos):
    ...
    methodlist = ['firewall_ipv6-eh-filter']
