...
DOCUMENTATION = '''
---
module: fortios_wireless-controller_qos-profile
short_description: Configure WiFi quality of service (QoS) profiles.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure wireless-controller feature and qos-profile category.
...
- hosts: localhost
  ...
  - name: Configure WiFi quality of service (QoS) profiles.
    fortios_wireless-controller_qos-profile:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_wireless-controller_qos-profile_data(json):
...
def wireless-controller_qos-profile(data, fos):
    vdom = data['vdom']
    wireless-controller_qos-profile_data = data['wireless-controller_qos-profile']
    filtered_data = filter_wireless-controller_qos - \
        profile_data(wireless-controller_qos-profile_data)

    if wireless-controller_qos-profile_data['state'] == "present":
        return fos.set('wireless-controller',
                       'qos-profile',
                       data=filtered_data,
                       vdom=vdom)

    elif wireless-controller_qos-profile_data['state'] == "absent":
...
def fortios_wireless-controller(data, fos):
    ...
    methodlist = ['wireless-controller_qos-profile']
