...
DOCUMENTATION = '''
---
module: fortios_firewall_ssl-ssh-profile
short_description: Configure SSL/SSH protocol options.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure firewall feature and ssl-ssh-profile category.
...
- hosts: localhost
  ...
  - name: Configure SSL/SSH protocol options.
    fortios_firewall_ssl-ssh-profile:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_firewall_ssl-ssh-profile_data(json):
...
def firewall_ssl-ssh-profile(data, fos):
    vdom = data['vdom']
    firewall_ssl-ssh-profile_data = data['firewall_ssl-ssh-profile']
    filtered_data = filter_firewall_ssl-ssh - \
        profile_data(firewall_ssl-ssh-profile_data)

    if firewall_ssl-ssh-profile_data['state'] == "present":
        return fos.set('firewall',
                       'ssl-ssh-profile',
                       data=filtered_data,
                       vdom=vdom)

    elif firewall_ssl-ssh-profile_data['state'] == "absent":
...
def fortios_firewall(data, fos):
    ...
    methodlist = ['firewall_ssl-ssh-profile']
