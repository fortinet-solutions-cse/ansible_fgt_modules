...
DOCUMENTATION = '''
---
module: fortios_switch-controller_global
short_description: Configure FortiSwitch global settings.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure switch-controller feature and global category.
...
- hosts: localhost
  ...
  - name: Configure FortiSwitch global settings.
    fortios_switch-controller_global:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_switch-controller_global_data(json):
...
def switch-controller_global(data, fos):
    vdom = data['vdom']
    switch-controller_global_data = data['switch-controller_global']
    filtered_data = filter_switch - \
        controller_global_data(switch-controller_global_data)

    if switch-controller_global_data['state'] == "present":
        return fos.set('switch-controller',
                       'global',
                       data=filtered_data,
                       vdom=vdom)

    elif switch-controller_global_data['state'] == "absent":
...
def fortios_switch-controller(data, fos):
    ...
    methodlist = ['switch-controller_global']
