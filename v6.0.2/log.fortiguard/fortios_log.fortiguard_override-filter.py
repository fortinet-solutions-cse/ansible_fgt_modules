...
DOCUMENTATION = '''
---
module: fortios_log.fortiguard_override-filter
short_description: Override filters for FortiCloud.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure log.fortiguard feature and override-filter category.
...
- hosts: localhost
  ...
  - name: Override filters for FortiCloud.
    fortios_log.fortiguard_override-filter:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_log.fortiguard_override-filter_data(json):
...
def log.fortiguard_override-filter(data, fos):
    vdom = data['vdom']
    log.fortiguard_override-filter_data = data['log.fortiguard_override-filter']
    filtered_data = filter_log.fortiguard_override - \
        filter_data(log.fortiguard_override-filter_data)

    if log.fortiguard_override-filter_data['state'] == "present":
        return fos.set('log.fortiguard',
                       'override-filter',
                       data=filtered_data,
                       vdom=vdom)

    elif log.fortiguard_override-filter_data['state'] == "absent":
...
def fortios_log.fortiguard(data, fos):
    ...
    methodlist = ['log.fortiguard_override-filter']
