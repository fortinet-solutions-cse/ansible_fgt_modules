...
- hosts: localhost
  ...
  - name: Global FortiAnalyzer settings.
    fortios_log.fortianalyzer3_setting:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_log.fortianalyzer3_setting_data(json):
...
def log.fortianalyzer3_setting(data, fos):
    ...
    if log.fortianalyzer3_setting_data['state'] == "present":
        return fos.set('log.fortianalyzer3',
                       'setting',
                       data=filtered_data,
                       vdom=vdom)

    elif log.fortianalyzer3_setting_data['state'] == "absent":
        return fos.delete('log.fortianalyzer3',
                          'setting',
                          mkey=filtered_data['id'],
                          vdom=vdom)


def fortios_log.fortianalyzer3(data, fos):
    host = data['host']
    username = data['username']
    password = data['password']
    fos.https('off')
    ...
    methodlist = ['log.fortianalyzer3_setting']
