...
DOCUMENTATION = '''
---
module: fortios_wanopt_content-delivery-network-rule
short_description: Configure WAN optimization content delivery network rules.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure wanopt feature and content-delivery-network-rule category.
...
- hosts: localhost
  ...
  - name: Configure WAN optimization content delivery network rules.
    fortios_wanopt_content-delivery-network-rule:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_wanopt_content-delivery-network-rule_data(json):
...
def wanopt_content-delivery-network-rule(data, fos):
    vdom = data['vdom']
    wanopt_content-delivery-network-rule_data = data['wanopt_content-delivery-network-rule']
    filtered_data = filter_wanopt_content-delivery-network - \
        rule_data(wanopt_content-delivery-network-rule_data)

    if wanopt_content-delivery-network-rule_data['state'] == "present":
        return fos.set('wanopt',
                       'content-delivery-network-rule',
                       data=filtered_data,
                       vdom=vdom)

    elif wanopt_content-delivery-network-rule_data['state'] == "absent":
...
def fortios_wanopt(data, fos):
    ...
    methodlist = ['wanopt_content-delivery-network-rule']
                              ...
                              "match-entries": {"required": False, "type": "list",
                                                "options": {
                                                    ...
                                                    "pattern": {"required": False, "type": "list",
                                                                "options": {
                                                                    "string": {"required": False, "type": "str"}
                                                                }},
                                                    "target": {"required": False, "type": "str",
                                                               "choices": ["path", "parameter", "referrer",
                                                                           "youtube-map", "youtube-id", "youku-id"]}
