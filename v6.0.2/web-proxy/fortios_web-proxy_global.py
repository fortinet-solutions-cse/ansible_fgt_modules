...
DOCUMENTATION = '''
---
module: fortios_web-proxy_global
short_description: Configure Web proxy global settings.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure web-proxy feature and global category.
...
def filter_web-proxy_global_data(json):
...
def web-proxy_global(data, fos):
    vdom = data['vdom']
    web-proxy_global_data = data['web-proxy_global']
    filtered_data = filter_web-proxy_global_data(web-proxy_global_data)

    if web-proxy_global_data['state'] == "present":
        return fos.set('web-proxy',
                       'global',
                       data=filtered_data,
                       vdom=vdom)

    elif web-proxy_global_data['state'] == "absent":
