...
DOCUMENTATION = '''
---
module: fortios_vpn.ssl.web_user-group-bookmark
short_description: Configure SSL VPN user group bookmark.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure vpn.ssl.web feature and user-group-bookmark category.
...
- hosts: localhost
  ...
  - name: Configure SSL VPN user group bookmark.
    fortios_vpn.ssl.web_user-group-bookmark:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_vpn.ssl.web_user-group-bookmark_data(json):
...
def vpn.ssl.web_user-group-bookmark(data, fos):
    vdom = data['vdom']
    vpn.ssl.web_user-group-bookmark_data = data['vpn.ssl.web_user-group-bookmark']
    filtered_data = filter_vpn.ssl.web_user-group - \
        bookmark_data(vpn.ssl.web_user-group-bookmark_data)

    if vpn.ssl.web_user-group-bookmark_data['state'] == "present":
        return fos.set('vpn.ssl.web',
                       'user-group-bookmark',
                       data=filtered_data,
                       vdom=vdom)

    elif vpn.ssl.web_user-group-bookmark_data['state'] == "absent":
...
def fortios_vpn.ssl.web(data, fos):
    ...
    methodlist = ['vpn.ssl.web_user-group-bookmark']
