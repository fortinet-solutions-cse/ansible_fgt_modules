...
DOCUMENTATION = '''
---
module: fortios_log.syslogd_override-setting
short_description: Override settings for remote syslog server.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure log.syslogd feature and override-setting category.
...
- hosts: localhost
  ...
  - name: Override settings for remote syslog server.
    fortios_log.syslogd_override-setting:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_log.syslogd_override-setting_data(json):
...
def log.syslogd_override-setting(data, fos):
    vdom = data['vdom']
    log.syslogd_override-setting_data = data['log.syslogd_override-setting']
    filtered_data = filter_log.syslogd_override - \
        setting_data(log.syslogd_override-setting_data)

    if log.syslogd_override-setting_data['state'] == "present":
        return fos.set('log.syslogd',
                       'override-setting',
                       data=filtered_data,
                       vdom=vdom)

    elif log.syslogd_override-setting_data['state'] == "absent":
        return fos.delete('log.syslogd',
                          'override-setting',
...
def fortios_log.syslogd(data, fos):
    ...
    methodlist = ['log.syslogd_override-setting']
