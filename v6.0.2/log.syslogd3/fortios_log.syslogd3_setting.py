...
DOCUMENTATION = '''
---
module: fortios_log.syslogd3_setting
short_description: Global settings for remote syslog server.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure log.syslogd3 feature and setting category.
...
def filter_log.syslogd3_setting_data(json):
...
def log.syslogd3_setting(data, fos):
    vdom = data['vdom']
    log.syslogd3_setting_data = data['log.syslogd3_setting']
    filtered_data = filter_log.syslogd3_setting_data(log.syslogd3_setting_data)

    if log.syslogd3_setting_data['state'] == "present":
        return fos.set('log.syslogd3',
                       'setting',
                       data=filtered_data,
                       vdom=vdom)

    elif log.syslogd3_setting_data['state'] == "absent":
        return fos.delete('log.syslogd3',
                          'setting',
                          mkey=filtered_data['id'],
                          vdom=vdom)


def fortios_log.syslogd3(data, fos):
    host = data['host']
    username = data['username']
    password = data['password']
    fos.https('off')
    ...
    methodlist = ['log.syslogd3_setting']
...
def main():
    ...
    is_error, has_changed, result = fortios_log.syslogd3(module.params, fos)
