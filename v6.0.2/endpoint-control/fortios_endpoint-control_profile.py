...
DOCUMENTATION = '''
---
module: fortios_endpoint-control_profile
short_description: Configure FortiClient endpoint control profiles.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure endpoint-control feature and profile category.
...
- hosts: localhost
  ...
  - name: Configure FortiClient endpoint control profiles.
    fortios_endpoint-control_profile:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_endpoint-control_profile_data(json):
...
def endpoint-control_profile(data, fos):
    vdom = data['vdom']
    endpoint-control_profile_data = data['endpoint-control_profile']
    filtered_data = filter_endpoint - \
        control_profile_data(endpoint-control_profile_data)

    if endpoint-control_profile_data['state'] == "present":
        return fos.set('endpoint-control',
                       'profile',
                       data=filtered_data,
                       vdom=vdom)

    elif endpoint-control_profile_data['state'] == "absent":
...
def fortios_endpoint-control(data, fos):
    ...
    methodlist = ['endpoint-control_profile']
    ...
    fields = {
        ...
        "endpoint-control_profile": {
            ...
            "options": {
                                                 ...
                                                 "options": {
                                                     ...
                                                     "forticlient-vpn-settings": {"required": False, "type": "list",
                                                                                  "options": {
                                                                                      ...
                                                                                      "name": {"required": True, "type": "str"},
                                                                                      "preshared-key": {"required": False, "type": "password"},
                ...
                "profile-name": {"required": False, "type": "str"},
