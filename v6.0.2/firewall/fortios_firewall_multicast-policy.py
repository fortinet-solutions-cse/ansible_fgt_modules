...
DOCUMENTATION = '''
---
module: fortios_firewall_multicast-policy
short_description: Configure multicast NAT policies.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure firewall feature and multicast-policy category.
...
- hosts: localhost
  ...
  - name: Configure multicast NAT policies.
    fortios_firewall_multicast-policy:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_firewall_multicast-policy_data(json):
...
def firewall_multicast-policy(data, fos):
    vdom = data['vdom']
    firewall_multicast-policy_data = data['firewall_multicast-policy']
    filtered_data = filter_firewall_multicast - \
        policy_data(firewall_multicast-policy_data)

    if firewall_multicast-policy_data['state'] == "present":
        return fos.set('firewall',
                       'multicast-policy',
                       data=filtered_data,
                       vdom=vdom)

    elif firewall_multicast-policy_data['state'] == "absent":
...
def fortios_firewall(data, fos):
    ...
    methodlist = ['firewall_multicast-policy']
        ...
        "firewall_multicast-policy": {
            ...
            "options": {
                ...
                "action": {"required": False, "type": "str",
                           "choices": ["accept", "deny"]},
                "dnat": {"required": False, "type": "ipv4-address-any"},
                ...
                "snat": {"required": False, "type": "str",
                         "choices": ["enable", "disable"]},
                "snat-ip": {"required": False, "type": "ipv4-address"},
