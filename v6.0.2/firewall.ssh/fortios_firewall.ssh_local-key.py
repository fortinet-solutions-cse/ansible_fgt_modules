...
DOCUMENTATION = '''
---
module: fortios_firewall.ssh_local-key
short_description: SSH proxy local keys.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure firewall.ssh feature and local-key category.
...
- hosts: localhost
  ...
  - name: SSH proxy local keys.
    fortios_firewall.ssh_local-key:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_firewall.ssh_local-key_data(json):
...
def firewall.ssh_local-key(data, fos):
    vdom = data['vdom']
    firewall.ssh_local-key_data = data['firewall.ssh_local-key']
    filtered_data = filter_firewall.ssh_local - \
        key_data(firewall.ssh_local-key_data)

    if firewall.ssh_local-key_data['state'] == "present":
        return fos.set('firewall.ssh',
                       'local-key',
                       data=filtered_data,
                       vdom=vdom)

    elif firewall.ssh_local-key_data['state'] == "absent":
...
def fortios_firewall.ssh(data, fos):
    ...
    methodlist = ['firewall.ssh_local-key']
