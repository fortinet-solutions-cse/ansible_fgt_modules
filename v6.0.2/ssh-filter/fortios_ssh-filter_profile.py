...
DOCUMENTATION = '''
---
module: fortios_ssh-filter_profile
short_description: SSH filter profile.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure ssh-filter feature and profile category.
...
def filter_ssh-filter_profile_data(json):
...
def ssh-filter_profile(data, fos):
    vdom = data['vdom']
    ssh-filter_profile_data = data['ssh-filter_profile']
    filtered_data = filter_ssh-filter_profile_data(ssh-filter_profile_data)

    if ssh-filter_profile_data['state'] == "present":
        return fos.set('ssh-filter',
                       'profile',
                       data=filtered_data,
                       vdom=vdom)

    elif ssh-filter_profile_data['state'] == "absent":
...
def fortios_ssh-filter(data, fos):
    host = data['host']
    username = data['username']
    password = data['password']
    fos.https('off')
...
def main():
    ...
    is_error, has_changed, result = fortios_ssh-filter(module.params, fos)
