...
DOCUMENTATION = '''
---
module: fortios_system_password-policy
...
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure system feature and password-policy category.
...
- hosts: localhost
  ...
  - name: Configure password policy for locally defined administrator passwords and IPsec VPN pre-shared keys.
    fortios_system_password-policy:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_system_password-policy_data(json):
...
def system_password-policy(data, fos):
    vdom = data['vdom']
    system_password-policy_data = data['system_password-policy']
    filtered_data = filter_system_password - \
        policy_data(system_password-policy_data)

    if system_password-policy_data['state'] == "present":
        return fos.set('system',
                       'password-policy',
                       data=filtered_data,
                       vdom=vdom)

    elif system_password-policy_data['state'] == "absent":
...
def fortios_system(data, fos):
    ...
    methodlist = ['system_password-policy']
