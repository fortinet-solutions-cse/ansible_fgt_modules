...
DOCUMENTATION = '''
---
module: fortios_dnsfilter_domain-filter
short_description: Configure DNS domain filters.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure dnsfilter feature and domain-filter category.
...
- hosts: localhost
  ...
  - name: Configure DNS domain filters.
    fortios_dnsfilter_domain-filter:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_dnsfilter_domain-filter_data(json):
...
def dnsfilter_domain-filter(data, fos):
    vdom = data['vdom']
    dnsfilter_domain-filter_data = data['dnsfilter_domain-filter']
    filtered_data = filter_dnsfilter_domain - \
        filter_data(dnsfilter_domain-filter_data)

    if dnsfilter_domain-filter_data['state'] == "present":
        return fos.set('dnsfilter',
                       'domain-filter',
                       data=filtered_data,
                       vdom=vdom)

    elif dnsfilter_domain-filter_data['state'] == "absent":
...
def fortios_dnsfilter(data, fos):
    ...
    methodlist = ['dnsfilter_domain-filter']
