...
DOCUMENTATION = '''
---
module: fortios_switch-controller_lldp-settings
short_description: Configure FortiSwitch LLDP settings.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure switch-controller feature and lldp-settings category.
...
- hosts: localhost
  ...
  - name: Configure FortiSwitch LLDP settings.
    fortios_switch-controller_lldp-settings:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_switch-controller_lldp-settings_data(json):
...
def switch-controller_lldp-settings(data, fos):
    vdom = data['vdom']
    switch-controller_lldp-settings_data = data['switch-controller_lldp-settings']
    filtered_data = filter_switch-controller_lldp - \
        settings_data(switch-controller_lldp-settings_data)

    if switch-controller_lldp-settings_data['state'] == "present":
        return fos.set('switch-controller',
                       'lldp-settings',
                       data=filtered_data,
                       vdom=vdom)

    elif switch-controller_lldp-settings_data['state'] == "absent":
...
def fortios_switch-controller(data, fos):
    ...
    methodlist = ['switch-controller_lldp-settings']
