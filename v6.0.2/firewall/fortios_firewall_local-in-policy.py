...
DOCUMENTATION = '''
---
module: fortios_firewall_local-in-policy
short_description: Configure user defined IPv4 local-in policies.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure firewall feature and local-in-policy category.
...
- hosts: localhost
  ...
  - name: Configure user defined IPv4 local-in policies.
    fortios_firewall_local-in-policy:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_firewall_local-in-policy_data(json):
...
def firewall_local-in-policy(data, fos):
    vdom = data['vdom']
    firewall_local-in-policy_data = data['firewall_local-in-policy']
    filtered_data = filter_firewall_local-in-policy_data(firewall_local-in-policy_data)

    if firewall_local-in-policy_data['state'] == "present":
        return fos.set('firewall',
                       'local-in-policy',
                       data=filtered_data,
                       vdom=vdom)

    elif firewall_local-in-policy_data['state'] == "absent":
...
def fortios_firewall(data, fos):
    ...
    methodlist = ['firewall_local-in-policy']
