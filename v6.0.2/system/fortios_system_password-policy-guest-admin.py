...
DOCUMENTATION = '''
---
module: fortios_system_password-policy-guest-admin
short_description: Configure the password policy for guest administrators.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure system feature and password-policy-guest-admin category.
...
- hosts: localhost
  ...
  - name: Configure the password policy for guest administrators.
    fortios_system_password-policy-guest-admin:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_system_password-policy-guest-admin_data(json):
...
def system_password-policy-guest-admin(data, fos):
    vdom = data['vdom']
    system_password-policy-guest-admin_data = data['system_password-policy-guest-admin']
    filtered_data = filter_system_password-policy-guest - \
        admin_data(system_password-policy-guest-admin_data)

    if system_password-policy-guest-admin_data['state'] == "present":
        return fos.set('system',
                       'password-policy-guest-admin',
                       data=filtered_data,
                       vdom=vdom)

    elif system_password-policy-guest-admin_data['state'] == "absent":
...
def fortios_system(data, fos):
    ...
    methodlist = ['system_password-policy-guest-admin']
