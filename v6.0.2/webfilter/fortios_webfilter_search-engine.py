...
DOCUMENTATION = '''
---
module: fortios_webfilter_search-engine
short_description: Configure web filter search engines.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure webfilter feature and search-engine category.
...
- hosts: localhost
  ...
  - name: Configure web filter search engines.
    fortios_webfilter_search-engine:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_webfilter_search-engine_data(json):
...
def webfilter_search-engine(data, fos):
    vdom = data['vdom']
    webfilter_search-engine_data = data['webfilter_search-engine']
    filtered_data = filter_webfilter_search - \
        engine_data(webfilter_search-engine_data)

    if webfilter_search-engine_data['state'] == "present":
        return fos.set('webfilter',
                       'search-engine',
                       data=filtered_data,
                       vdom=vdom)

    elif webfilter_search-engine_data['state'] == "absent":
...
def fortios_webfilter(data, fos):
    ...
    methodlist = ['webfilter_search-engine']
