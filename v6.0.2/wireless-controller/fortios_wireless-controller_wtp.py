...
DOCUMENTATION = '''
---
module: fortios_wireless-controller_wtp
...
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure wireless-controller feature and wtp category.
...
- hosts: localhost
  ...
  - name: Configure Wireless Termination Points (WTPs), that is, FortiAPs or APs to be managed by FortiGate.
    fortios_wireless-controller_wtp:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_wireless-controller_wtp_data(json):
...
def wireless-controller_wtp(data, fos):
    vdom = data['vdom']
    wireless-controller_wtp_data = data['wireless-controller_wtp']
    filtered_data = filter_wireless - \
        controller_wtp_data(wireless-controller_wtp_data)

    if wireless-controller_wtp_data['state'] == "present":
        return fos.set('wireless-controller',
                       'wtp',
                       data=filtered_data,
                       vdom=vdom)

    elif wireless-controller_wtp_data['state'] == "absent":
...
def fortios_wireless-controller(data, fos):
    ...
    methodlist = ['wireless-controller_wtp']
    ...
    fields = {
        ...
        "wireless-controller_wtp": {
            ...
            "options": {
                ...
                "wtp-id": {"required": False, "type": "str"},
