...
DOCUMENTATION = '''
---
module: fortios_wireless-controller.hotspot20_h2qp-wan-metric
short_description: Configure WAN metrics.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure wireless-controller.hotspot20 feature and h2qp-wan-metric category.
...
- hosts: localhost
  ...
  - name: Configure WAN metrics.
    fortios_wireless-controller.hotspot20_h2qp-wan-metric:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_wireless-controller.hotspot20_h2qp-wan-metric_data(json):
...
def wireless-controller.hotspot20_h2qp-wan-metric(data, fos):
    vdom = data['vdom']
    wireless-controller.hotspot20_h2qp-wan-metric_data = data['wireless-controller.hotspot20_h2qp-wan-metric']
    filtered_data = filter_wireless-controller.hotspot20_h2qp-wan - \
        metric_data(wireless-controller.hotspot20_h2qp-wan-metric_data)

    if wireless-controller.hotspot20_h2qp - \
            wan-metric_data['state'] == "present":
        return fos.set('wireless-controller.hotspot20',
                       'h2qp-wan-metric',
                       data=filtered_data,
                       vdom=vdom)

    elif wireless-controller.hotspot20_h2qp-wan-metric_data['state'] == "absent":
...
def fortios_wireless-controller.hotspot20(data, fos):
    ...
    methodlist = ['wireless-controller.hotspot20_h2qp-wan-metric']
