...
DOCUMENTATION = '''
---
module: fortios_extender-controller_extender
short_description: Extender controller configuration.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure extender-controller feature and extender category.
...
- hosts: localhost
  ...
  - name: Extender controller configuration.
    fortios_extender-controller_extender:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_extender-controller_extender_data(json):
...
def extender-controller_extender(data, fos):
    vdom = data['vdom']
    extender-controller_extender_data = data['extender-controller_extender']
    filtered_data = filter_extender - \
        controller_extender_data(extender-controller_extender_data)

    if extender-controller_extender_data['state'] == "present":
        return fos.set('extender-controller',
                       'extender',
                       data=filtered_data,
                       vdom=vdom)

    elif extender-controller_extender_data['state'] == "absent":
...
def fortios_extender-controller(data, fos):
    host = data['host']
    username = data['username']
    password = data['password']
    fos.https('off')
    ...
    methodlist = ['extender-controller_extender']
...
def main():
    fields = {
        ...
        "extender-controller_extender": {
            ...
            "options": {
                ...
                "aaa-shared-secret": {"required": False, "type": "password"},
                ...
                "ha-shared-secret": {"required": False, "type": "password"},
                ...
                "modem-passwd": {"required": False, "type": "password"},
                ...
                "ppp-echo-request": {"required": False, "type": "str",
                                     "choices": ["enable", "disable"]},
                "ppp-password": {"required": False, "type": "password"},
                ...
                "sim-pin": {"required": False, "type": "password"},
    ...
    is_error, has_changed, result = fortios_extender - \
