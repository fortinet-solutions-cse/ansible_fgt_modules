...
DOCUMENTATION = '''
---
module: fortios_web-proxy_explicit
short_description: Configure explicit Web proxy settings.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure web-proxy feature and explicit category.
...
def filter_web-proxy_explicit_data(json):
...
def web-proxy_explicit(data, fos):
    vdom = data['vdom']
    web-proxy_explicit_data = data['web-proxy_explicit']
    filtered_data = filter_web-proxy_explicit_data(web-proxy_explicit_data)

    if web-proxy_explicit_data['state'] == "present":
        return fos.set('web-proxy',
                       'explicit',
                       data=filtered_data,
                       vdom=vdom)

    elif web-proxy_explicit_data['state'] == "absent":
    ...
    fields = {
        ...
        "web-proxy_explicit": {
            ...
            "options": {
                ...
                "incoming-ip6": {"required": False, "type": "ipv6-address"},
                ...
                "outgoing-ip6": {"required": False, "type": "ipv6-address"},
