...
DOCUMENTATION = '''
---
module: fortios_wireless-controller.hotspot20_anqp-ip-address-type
short_description: Configure IP address type availability.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure wireless-controller.hotspot20 feature and anqp-ip-address-type category.
...
- hosts: localhost
  ...
  - name: Configure IP address type availability.
    fortios_wireless-controller.hotspot20_anqp-ip-address-type:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_wireless-controller.hotspot20_anqp-ip-address-type_data(json):
...
def wireless-controller.hotspot20_anqp-ip-address-type(data, fos):
    vdom = data['vdom']
    wireless-controller.hotspot20_anqp-ip-address-type_data = data['wireless-controller.hotspot20_anqp-ip-address-type']
    filtered_data = filter_wireless-controller.hotspot20_anqp-ip-address - \
        type_data(wireless-controller.hotspot20_anqp-ip-address-type_data)

    if wireless-controller.hotspot20_anqp-ip - \
            address-type_data['state'] == "present":
        return fos.set('wireless-controller.hotspot20',
                       'anqp-ip-address-type',
                       data=filtered_data,
                       vdom=vdom)

    elif wireless-controller.hotspot20_anqp-ip-address-type_data['state'] == "absent":
...
def fortios_wireless-controller.hotspot20(data, fos):
    ...
    methodlist = ['wireless-controller.hotspot20_anqp-ip-address-type']
