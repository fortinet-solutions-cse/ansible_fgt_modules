...
DOCUMENTATION = '''
---
module: fortios_log.syslogd3_filter
short_description: Filters for remote system server.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure log.syslogd3 feature and filter category.
...
def filter_log.syslogd3_filter_data(json):
...
def log.syslogd3_filter(data, fos):
    vdom = data['vdom']
    log.syslogd3_filter_data = data['log.syslogd3_filter']
    filtered_data = filter_log.syslogd3_filter_data(log.syslogd3_filter_data)

    if log.syslogd3_filter_data['state'] == "present":
        return fos.set('log.syslogd3',
                       'filter',
                       data=filtered_data,
                       vdom=vdom)

    elif log.syslogd3_filter_data['state'] == "absent":
...
def fortios_log.syslogd3(data, fos):
    ...
    methodlist = ['log.syslogd3_filter']
