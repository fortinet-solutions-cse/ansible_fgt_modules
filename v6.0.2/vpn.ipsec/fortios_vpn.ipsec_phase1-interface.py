...
DOCUMENTATION = '''
---
module: fortios_vpn.ipsec_phase1-interface
short_description: Configure VPN remote gateway.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure vpn.ipsec feature and phase1-interface category.
...
- hosts: localhost
  ...
  - name: Configure VPN remote gateway.
    fortios_vpn.ipsec_phase1-interface:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_vpn.ipsec_phase1-interface_data(json):
...
def vpn.ipsec_phase1-interface(data, fos):
    vdom = data['vdom']
    vpn.ipsec_phase1-interface_data = data['vpn.ipsec_phase1-interface']
    filtered_data = filter_vpn.ipsec_phase1 - \
        interface_data(vpn.ipsec_phase1-interface_data)

    if vpn.ipsec_phase1-interface_data['state'] == "present":
        return fos.set('vpn.ipsec',
                       'phase1-interface',
                       data=filtered_data,
                       vdom=vdom)

    elif vpn.ipsec_phase1-interface_data['state'] == "absent":
...
def fortios_vpn.ipsec(data, fos):
    ...
    methodlist = ['vpn.ipsec_phase1-interface']
    ...
    fields = {
        ...
        "vpn.ipsec_phase1-interface": {
            ...
            "options": {
                ...
                "default-gw": {"required": False, "type": "ipv4-address"},
                ...
                "encap-local-gw4": {"required": False, "type": "ipv4-address"},
                "encap-local-gw6": {"required": False, "type": "ipv6-address"},
                "encap-remote-gw4": {"required": False, "type": "ipv4-address"},
                "encap-remote-gw6": {"required": False, "type": "ipv6-address"},
