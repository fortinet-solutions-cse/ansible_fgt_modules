...
DOCUMENTATION = '''
---
module: fortios_user_device-access-list
short_description: Configure device access control lists.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure user feature and device-access-list category.
...
- hosts: localhost
  ...
  - name: Configure device access control lists.
    fortios_user_device-access-list:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_user_device-access-list_data(json):
...
def user_device-access-list(data, fos):
    vdom = data['vdom']
    user_device-access-list_data = data['user_device-access-list']
    filtered_data = filter_user_device-access - \
        list_data(user_device-access-list_data)

    if user_device-access-list_data['state'] == "present":
        return fos.set('user',
                       'device-access-list',
                       data=filtered_data,
                       vdom=vdom)

    elif user_device-access-list_data['state'] == "absent":
...
def fortios_user(data, fos):
    ...
    methodlist = ['user_device-access-list']
