...
DOCUMENTATION = '''
---
module: fortios_firewall_shaping-policy
short_description: Configure shaping policies.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure firewall feature and shaping-policy category.
...
- hosts: localhost
  ...
  - name: Configure shaping policies.
    fortios_firewall_shaping-policy:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_firewall_shaping-policy_data(json):
...
def firewall_shaping-policy(data, fos):
    vdom = data['vdom']
    firewall_shaping-policy_data = data['firewall_shaping-policy']
    filtered_data = filter_firewall_shaping - \
        policy_data(firewall_shaping-policy_data)

    if firewall_shaping-policy_data['state'] == "present":
        return fos.set('firewall',
                       'shaping-policy',
                       data=filtered_data,
                       vdom=vdom)

    elif firewall_shaping-policy_data['state'] == "absent":
...
def fortios_firewall(data, fos):
    ...
    methodlist = ['firewall_shaping-policy']
