...
DOCUMENTATION = '''
---
module: fortios_log.memory_global-setting
short_description: Global settings for memory logging.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure log.memory feature and global-setting category.
...
- hosts: localhost
  ...
  - name: Global settings for memory logging.
    fortios_log.memory_global-setting:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_log.memory_global-setting_data(json):
...
def log.memory_global-setting(data, fos):
    vdom = data['vdom']
    log.memory_global-setting_data = data['log.memory_global-setting']
    filtered_data = filter_log.memory_global - \
        setting_data(log.memory_global-setting_data)

    if log.memory_global-setting_data['state'] == "present":
        return fos.set('log.memory',
                       'global-setting',
                       data=filtered_data,
                       vdom=vdom)

    elif log.memory_global-setting_data['state'] == "absent":
        return fos.delete('log.memory',
                          'global-setting',
...
def fortios_log.memory(data, fos):
    ...
    methodlist = ['log.memory_global-setting']
