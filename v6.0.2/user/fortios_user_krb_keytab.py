...
DOCUMENTATION = '''
---
module: fortios_user_krb_keytab
short_description: Configure Kerberos keytab entries in Fortinet's FortiOS and FortiGate.
description:
    - This module is able to configure a FortiGate or FortiOS by allowing the
      user to set and modify user feature and krb_keytab category.
...
def filter_user_krb_keytab_data(json):
    option_list = ['keytab', 'ldap-server', 'name',
                   'principal']
    dictionary = {}
...
def user_krb_keytab(data, fos):
    vdom = data['vdom']
    user_krb_keytab_data = data['user_krb_keytab']
    flattened_data = flatten_multilists_attributes(user_krb_keytab_data)
    filtered_data = filter_user_krb_keytab_data(flattened_data)
    if user_krb_keytab_data['state'] == "present":
        return fos.set('user',
                       'krb-keytab',
                       data=filtered_data,
                       vdom=vdom)

    elif user_krb_keytab_data['state'] == "absent":
