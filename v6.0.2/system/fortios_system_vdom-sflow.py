...
DOCUMENTATION = '''
---
module: fortios_system_vdom-sflow
short_description: Configure sFlow per VDOM to add or change the IP address and UDP port that FortiGate sFlow agents in this VDOM use to send sFlow datagrams to an sFlow collector.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure system feature and vdom-sflow category.
...
def filter_system_vdom-sflow_data(json):
...
def system_vdom-sflow(data, fos):
    vdom = data['vdom']
    system_vdom-sflow_data = data['system_vdom-sflow']
    filtered_data = filter_system_vdom-sflow_data(system_vdom-sflow_data)

    if system_vdom-sflow_data['state'] == "present":
        return fos.set('system',
                       'vdom-sflow',
                       data=filtered_data,
                       vdom=vdom)

    elif system_vdom-sflow_data['state'] == "absent":
