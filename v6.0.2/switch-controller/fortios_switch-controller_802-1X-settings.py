...
DOCUMENTATION = '''
---
module: fortios_switch-controller_802-1X-settings
short_description: Configure global 802.1X settings.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure switch-controller feature and 802-1X-settings category.
...
- hosts: localhost
  ...
  - name: Configure global 802.1X settings.
    fortios_switch-controller_802-1X-settings:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_switch-controller_802-1X-settings_data(json):
...
def switch-controller_802-1X-settings(data, fos):
    vdom = data['vdom']
    switch-controller_802-1X-settings_data = data['switch-controller_802-1X-settings']
    filtered_data = filter_switch-controller_802-1X-settings_data(switch-controller_802-1X-settings_data)

    if switch-controller_802-1X-settings_data['state'] == "present":
        return fos.set('switch-controller',
                       '802-1X-settings',
                       data=filtered_data,
                       vdom=vdom)

    elif switch-controller_802-1X-settings_data['state'] == "absent":
...
def fortios_switch-controller(data, fos):
    ...
    methodlist = ['switch-controller_802-1X-settings']
