...
DOCUMENTATION = '''
---
module: fortios_log.syslogd_override-filter
short_description: Override filters for remote system server.
description:
    - This module is able to configure a FortiGate or FortiOS by
      allowing the user to configure log.syslogd feature and override-filter category.
...
- hosts: localhost
  ...
  - name: Override filters for remote system server.
    fortios_log.syslogd_override-filter:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def filter_log.syslogd_override-filter_data(json):
...
def log.syslogd_override-filter(data, fos):
    vdom = data['vdom']
    log.syslogd_override-filter_data = data['log.syslogd_override-filter']
    filtered_data = filter_log.syslogd_override - \
        filter_data(log.syslogd_override-filter_data)

    if log.syslogd_override-filter_data['state'] == "present":
        return fos.set('log.syslogd',
                       'override-filter',
                       data=filtered_data,
                       vdom=vdom)

    elif log.syslogd_override-filter_data['state'] == "absent":
...
def fortios_log.syslogd(data, fos):
    ...
    methodlist = ['log.syslogd_override-filter']
